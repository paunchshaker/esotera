class Resource:
    ...
    def __mul__(self, other):
        """Multiply the quantity of a resource by the quantity of the same kind or by a number"""
        value = other
        if isinstance(other, Resource):
            if other.kind == self.kind:
                value = other.quantity
    ...
    def __floordiv__(self, other):
        """Divide the quantity of a resource by the quantity of the same kind or by a number"""
        value = other
        if isinstance(other, Resource):
            if other.kind == self.kind:
                value = other.quantity
